...
def read_flagged(folder: Path) -> dict[str, dict]:
    ...
    for i, f in enumerate(files, 1):
        ...
        flagged[f.stem[len("process_"):]] = {
            ...
            "source": " | ".join(x for x in (src.get("publisher"), src.get("year"), src.get("title")) if x) if src is not None else "",
        }
    return flagged
...
def main() -> int:
    ...
    fields = [
        "code", "name", "location", "unit", "bafu_category",
        "n_technosphere_inputs", "n_elementary_flows", "n_used_by", "generator", "source",
    ]
