...
Dataset: `{{target_name}}` [{{target_location}}], 1 {{target_unit}}, BAFU category {{target_category}}.
Metadata: includedProcesses: {{included_processes}} · technology: {{technology}} · comment: {{general_comment}}

Report: `{{report_name}}` ({{n_pages}} PDF pages). Below is every table and figure caption found in it, with the PDF page it appears on (not the printed page number). The report may be in German, French or English; the dataset name is English — match on meaning (e.g. "gebrannter Ölschiefer" = burnt shale, "Gipsfaserplatte" = gypsum fibre board).
