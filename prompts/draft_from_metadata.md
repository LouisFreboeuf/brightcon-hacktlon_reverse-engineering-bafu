...
The dataset `{{target_name}}` [{{target_location}}], reference unit 1 {{target_unit}}, is shipped in the BAFU-2026 database as a system process: it has no production inputs, only ~{{n_flows}} cumulative elementary flows. No report in the documentation bundle prints its inventory, and no unit process of the same product was found to copy:
...
2. Give every item exactly one `basis` for its amount, per 1 {{target_unit}} of product:
