...
Dataset: `Use, computer, laptop, standby/sleep mode` [RER], 1 hr, BAFU category computers & network / use, obsolete\computers & network.
Metadata: includedProcesses: Describes the use of a laptop computer in the standby/sleep mode. Calculated per 1 hour of laptop use. Laptop computer life span assumed to be 4 years. Standby/sleep mode 2 hours a day (office use), 240 days a year. UCTE production mix applied for electricity consumption (low voltage). · technology: No technology involved, except for the production of the laptop computer, which is described in another dataset. · comment: The laptop use in the standby/sleep mode includes the share of the laptop computer production, the energy consumption (use-phase) for the laptop (incl. monitor) and the rail and road transport from factory to user. Energy consumption of laptop computers (use-phase) vary a lot depending on the device
